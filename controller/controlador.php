...
class PDF extends FPDF{
    ...
    $this->Text(20,14,'Historial sucursal-SR-Solutions',0,'C', 0);
    $this->Ln(10);
    }
    ...
    $this->Cell(100,10,'Historial',0,0,'L');
    ...
    $objpaciente = new sucursales();
    // obtenemos los datos del paciente por el id
    $sucursal = $objpaciente->sucursal_id($id);
    ...
    $pdf->Cell(0,6,$sucursal['id'],0,1);
    $pdf->Cell(30,6,'NOMBRE: ',0,0);
    $pdf->Cell(0,6,$sucursal['ciudad'].' '.$paciente['teleforno'].' '.$paciente['correo'],0,1);
    ...
    $pdf->Image(Conexion::ruta().'../UB-XYZ.jpg'.$paciente['foto'],155,20,30,30);
    ...
    $pdf->Row(array('id','cod_Ciudad', 'cod_Empresa', 'direccion', 'teleforno', 'correo', 'estado'));
    // Creamos nuestra funcion consulta
    $objconsulta = new model();
    $consultas = $objconsulta->consultasSucursal($id);
    ...
    $pdf->SetTextColor(0);
    $pdf->Row(array($n,$consulta['id'], $consulta['cod_Ciudad'], $consulta['cod_Empresa'], $consulta['direccion']));
    $i++;
    }else{
